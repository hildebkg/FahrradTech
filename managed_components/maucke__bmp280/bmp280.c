...
static esp_err_t bmp280_read_uint16_le(bmp280_handle_t sensor, uint8_t addr, uint16_t *data)
{
    ...
    if (i2c_bus_read_byte(sens->i2c_dev, addr, &data0) != ESP_OK)
    {
        ESP_LOGI("BMP280:", "bmp280_read_uint16_le->i2c_bus_read_byte failed to read byte at address 0x%x", addr);
        return ret;
    }
    if (i2c_bus_read_byte(sens->i2c_dev, addr + 1, &data1) != ESP_OK)
    {
        ESP_LOGI("BMP280:", "bmp280_read_uint16_le->i2c_bus_read_byte failed to read byte at address 0x%x", addr + 1);
        return ret;
    }
    *data = (data1 << 8) | data0;
    return ESP_OK;
...
esp_err_t bmp280_default_init(bmp280_handle_t sensor)
{
    ...
    {
        ESP_LOGI("BMP280:", "bmp280_default_init->bmp280_read_byte ->BMP280_REGISTER_CHIPID failed!!!!: 0x%x", chipid);
        return ESP_FAIL;
    }
    if (chipid != BMP280_DEFAULT_CHIPID)
    {
        ESP_LOGI("BMP280:", "bmp280_default_init->BMP280_DEFAULT_CHIPID: 0x%x", chipid);
        return ESP_FAIL;
    }
    ...
    {
        ESP_LOGI("BMP280:", "bmp280_default_init->i2c_bus_write_byte for soft reset failed!!!");
        return ESP_FAIL;
    }
    ...
    { // read trimming parameters, see DS 4.2.2
        ESP_LOGI("BMP280:", "bmp280_default_init->bmp280_read_coefficients failed!!!");
        return ESP_FAIL;
    }
    ...
    { // use defaults
        ESP_LOGI("BMP280:", "bmp280_default_init->bmp280_set_sampling failed!!!");
        return ESP_FAIL;
    }
    return ESP_OK;
}
...
esp_err_t bmp280_read_altitude(bmp280_handle_t sensor, float seaLevel, float *altitude)
{
    ...
    if (bmp280_read_pressure(sensor, &pressure) != ESP_OK)
    {
        return ESP_FAIL;
    }
